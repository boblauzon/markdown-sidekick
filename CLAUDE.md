...
The passes were tuned against a real 25-book PDF-conversion corpus; `tests/test_cleanup.py` encodes those artifact shapes — run it after any cleanup change. Fence language labels come from `_guess_language` (content scoring; returns `""` plain fence when ambiguous — an honest plain fence beats a wrong label).
...
**Settings** (`settings.py`): dataclass persisted to `%LOCALAPPDATA%\MarkdownSidekick\settings.json`; load is fully tolerant of junk. Whisper models are materialised as real file copies under the same dir (`local_dir=`, not the HF symlink cache — Windows symlink privilege issue).
