...
Adding a data file or a new heavy dependency usually means touching three places: `pyproject.toml` (package-data / dependencies), `MarkdownSidekick.spec` (datas / collect_all), and `app.py --selftest` if it's pipeline-critical. RapidOCR v3 downloads its ONNX models into site-packages on first use — run one OCR (e.g. `app.py --selftest`) in the build venv **before** PyInstaller so `collect_all("rapidocr")` bundles them; the frozen selftest's `image_ocr` check proves it worked. OCR device selection: `Settings.ocr_device` (auto/cpu/gpu) — "auto" resolves DirectML from `onnxruntime.get_available_providers()` in `ocr.py` because RapidOCR's own default silently stays on CPU. `USERGUIDE.md` ships inside the package (loaded via `importlib.resources`) — keep its links unwrapped by bold.
